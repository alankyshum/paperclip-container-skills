...
| `scripts/search-web.py` | Perplexity + Gemini web search with citations. **Standard-library only — no `pip`/`requests` needed.** | `PERPLEXITY_API_KEY`, `SEARCH_API_KEY` |
| `scripts/context7-tool.py` | Context7 library documentation lookup. | `CONTEXT7_API_KEY` |
| `scripts/requirements.txt` | Python deps. Only `context7-tool.py` needs `requests`; `search-web.py` is stdlib-only. |  |

## Install (Dockerfile pattern)

```dockerfile
# python3-requests is only needed by context7-tool.py; search-web.py is stdlib-only.
...
RUN git clone --depth 1 https://github.com/alankyshum/paperclip-container-skills.git /tmp/skills && \
    ...
    chmod +x /skills/scripts/clip.sh /skills/scripts/memory-cli /skills/scripts/search-web.py && \
    rm -rf /tmp/skills
```
